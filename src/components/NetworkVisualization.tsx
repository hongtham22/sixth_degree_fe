...
export const NetworkVisualization: React.FC<NetworkVisualizationProps> = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Fetch full graph adjacency from api/graph
      try {
  ...
  // Clear previous visualization when a new search starts
  useEffect(() => {
  ...
  useEffect(() => {
    const g = graphRef.current;
    if (!g || !cacheGraph.current) return;

    // Only add explored nodes
    exploredNodes.forEach((id) => {
      if (!g.hasNode(id)) {
        ...
        // Add edges from graph if available
        const neighbors = cacheGraph.current[id] || [];
        neighbors.forEach((neighbor) => {
          if (g.hasNode(neighbor) && !g.hasEdge(id, neighbor)) {
            g.addEdge(id, neighbor, { color: '#4B5563', size: 0.5 });
          }
        });
      }
    });
